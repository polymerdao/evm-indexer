...
async function getPolymerData(query: SearchTxQuery, eventType: string): Promise<PolymerData | null> {
  ...
  const end = Date.now();
  if ((end - start) / 1000 > 1) {
    console.log(`getCosmosPolymerData took ${(end - start) / 1000} seconds`);
  }
  if (polymerData) {
    return polymerData
  }
  ...
  try {
    ...
    if ((endSearch - startSearch) / 1000 > 1) {
      console.log(`stargateClient.searchTx took ${(endSearch - startSearch) / 1000} seconds`);
    }
  } catch (e) {
